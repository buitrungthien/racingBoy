...
@Component({
    ...
    styleUrls: ['./homepage.component.css', './../../../node_modules/flag-icon-css/css/flag-icon.min.css']
})
export class HomepageComponent implements OnInit {
    ...
    fromLargerToSmaller = false;
    nationalityMappingTable = [
        {
            "num_code": "4",
            "alpha_2_code": "AF",
            "alpha_3_code": "AFG",
    ...
    constructor(private allRacingBoys: racingBoyService) { }

    ngOnInit() {
        this.showAllRacingBoys();
    }

    showAllRacingBoys() {
        this.allRacingBoys.getAllRacingBoys()
            .subscribe((res: {}) => {
                var fetchedData = res['MRData'].StandingsTable.StandingsLists[0].DriverStandings;
                this.tableOfRiders = fetchedData.map(element => {
                    return {
                        position: element['position'],
                        nationality: element['Driver']['nationality'],
                        fullName: `${element['Driver']['givenName']} ${element['Driver']['familyName']}`,
                        team: element['Constructors'][0]['name'],
    ...
    displayFlag(nationality: string) {
        const result = this.nationalityMappingTable.filter(country => {
            return country.nationality.includes(nationality);
        });
        return `flag-icon flag-icon-${result[0].alpha_2_code.toLowerCase()}`;
    }
}
